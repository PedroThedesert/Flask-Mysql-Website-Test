...
app.secret_key = 'clave_secreta_flask' # crear la clave secreta para los mensajes flash
...
@app.route('/insertar_coche_sql') # Insertar SQL COCHE
def insertar_coche_sql ():
    cursor = mysql.connection.cursor()  # te conecta al cursor y lanza consultas
    ...
    return redirect(url_for('index')) # Hacemos redireccion después de commit

@app.route('/formulario_crear_coche_sql' , methods=['GET', 'POST']) # Insertar SQL COCHE en FORMULARIO
def crear_coche_formulario_sql (): # http://127.0.0.1:5000/formulario_crear_coche_sql
    if request.method == 'POST':   # si me llega request por post ejecutaremos

        # recepcion del formulario html
        ...
        cursor.connection.commit()
        flash('Has creado el coche')
        print((marca, modelo, precio, ciudad))

        # return f"{marca} {modelo} {precio} {ciudad}"  IMPRIME LOS DATOS POR WEB

        return redirect(url_for('index_flash')) # Hacemos redireccion después de commit

    # Si no llegan datos por post hacemos un render template del formulario
    return render_template('form_crear_coche.html')
...
@app.route('/index_mensajes_flash') # vista genereal
def index_flash():
    return render_template('Index_mensajes_flash.html')
